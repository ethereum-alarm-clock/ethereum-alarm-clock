...
def handle_transaction_request(config, txn_request):
    ...
    if txn_request.isCancelled:
        config.debug("Ignoring Cancelled Request @ %s", txn_request.address)
        return
    elif txn_request.beforeClaimWindow:
        config.debug("Ignoring Pending Request @ %s", txn_request.address)
        return
    elif txn_request.isFrozen:
        config.debug("Ignoring Frozen Request @ %s", txn_request.address)
        return

    if txn_request.isClaimable:
...
def request_create(ctx,
    ...
    while True:
        try:
            ...
            create_txn_receipt = wait.for_receipt(create_txn_hash)
