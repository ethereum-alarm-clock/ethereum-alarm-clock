...
contract('Timestamp reserved window', async function(accounts) {
    ...
    it('should reject execution if claimed by another', async function() {
        ...
        const secs = requestData.schedule.windowStart - (await config.web3.eth.getBlock('latest')).timestamp
        ...
        expect(parseAbortData(failedExecuteTx).find(reason => reason === 'ReservedForClaimer'))
        .to.exist
