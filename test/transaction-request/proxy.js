...
contract('TransactionRequest proxy function', accounts => {
    ...
    it('only allows calling proxy after execution window and by the scheduling account', async () => {
        ...
        const duringExecutionWindow = requestData.schedule.windowStart + requestData.schedule.windowSize - 2

        await waitUntilBlock(0, duringExecutionWindow)

        /// This fails because it is not after the exeucution window
        await txRequest.proxy(accounts[7], testData32)
        .should.be.rejectedWith('VM Exception while processing transaction: revert')

        const afterExecutionWindow = requestData.schedule.windowStart + requestData.schedule.windowSize + 1

        await waitUntilBlock(0, afterExecutionWindow)

        /// This throws because it is not the scheduling account
        await txRequest.proxy(accounts[7], testData32, {from: accounts[4]})
        .should.be.rejectedWith('VM Exception while processing transaction: revert')

        /// This is allowed since it is from scheduling accounts
        const tx = await txRequest.proxy(accounts[7], testData32)
        expect(tx.receipt.status)
        .to.equal(1)
    })
    ...
    it('does some fancy stuff with proxy', async () => {
        ...
        expect( (await tokenContract.balanceOf(txRequest.address)).toNumber() )
        .to.equal(12345 * 30) //callValue * rate

        const afterExecutionWindow = requestData.schedule.windowStart + requestData.schedule.windowSize + 2
        ...
        expect( (await tokenContract.balanceOf(accounts[8])).toNumber() )
        .to.equal(30000)
