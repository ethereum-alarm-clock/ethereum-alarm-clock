...
def get_balance_delta(deploy_client, txn_hash):
    coinbase = deploy_client.get_coinbase()
    txn = deploy_client.get_transaction_by_hash(txn_hash)
    before_txn_balance = deploy_client.get_balance(coinbase, int(txn['blockNumber'], 16) - 1)
    after_txn_balance = deploy_client.get_balance(coinbase, txn['blockNumber'])
...
def test_withdrawing_funds(deploy_client, deployed_contracts):
    ...
    txn_1_hash = alarm.deposit.sendTransaction(coinbase, value=1000 * denoms.ether)
