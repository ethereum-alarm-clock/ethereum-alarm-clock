...
def test_pool_membership_frozen_during_transition_period(deploy_client,
    ...
    deploy_client.wait_for_transaction(joiner.setCallerPool.sendTransaction(scheduler._meta.address))
    ...
    deploy_client.wait_for_block(
        scheduler.getGenerationStartAt(second_generation_id) - freeze_duration, 180,
    )
